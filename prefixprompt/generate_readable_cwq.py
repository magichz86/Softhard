...
def main(generation_arguments):
    # load dataset
    # load dataset
    raw_dataset = load_data("../data/ComplexWebQuestions/CWQ.json")
    dataset = {}
    dataset["test"] = []
    for data in raw_dataset["test"]:
        input_example = InputExample(guid=data["guid"], text_a=data['seq'], text_b="",
                                     tgt_text=data['question'])
        dataset["test"].append(input_example)
    print("This is the first example of set:")
    ...
    mytemplate = PrefixTuningTemplate(model=plm,  tokenizer=tokenizer, text=' {"placeholder":"text_a"} {"special": "<eos>"} {"mask"} ', num_token=args.num_token, using_decoder_past_key_values=False)
    ...
    #     torch.save(prompt_model.state_dict(), './saved_dict/' + "webnlg_Epoch" + str(epoch) + "_" + args.model_name_or_path + '.pt')

    prompt_model.load_state_dict(torch.load('./saved_dict/webnlg_Epoch4_t5-base.pt'))
    generated_sentence, guid_num = evaluate(prompt_model, test_dataloader, generation_arguments)

    with open(f"Generated_text_cwq2.json", 'w') as f:
        for i, s in enumerate(generated_sentence):
            dump = {"generated_sentence": s, "guid": guid_num[i]}
...
def evaluate(prompt_model, dataloader, generation_arguments):
    ...
    for step, inputs in enumerate(dataloader):
        ...
        guid_num.extend(inputs['guid'].tolist())
        if step % 500 == 0:
            print("evaluate step " + str(step))
        # print(guid_num)
        # print(len(guid_num))
    print("Here is an example of the generated sentence:")
    ...
    return generated_sentence, guid_num
